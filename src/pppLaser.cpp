...
STATIC_ASSERT(offsetof(LaserMapCylinder, m_axis) == offsetof(CMapCylinder, m_axis));
STATIC_ASSERT(offsetof(LaserMapCylinder, m_boundsMin) == offsetof(CMapCylinder, m_boundsMin));
STATIC_ASSERT(offsetof(LaserMapCylinder, m_boundsMax) == offsetof(CMapCylinder, m_boundsMax));
...
extern "C" void pppFrameLaser(struct pppLaser *pppLaser, struct pppLaserUnkB *param_2, _pppCtrlTable *param_3)
{
    ...
    for (int i = 0; i < (int)(u32)(step->m_laser.m_historyFrameCount + 1); i++) {
        ...
        if (i == 0) {
            ...
            float t = LaserConst(kPppLaserNegativeOne) / (float)frameCount;
            t *= (float)i;
...
extern "C" void pppRenderLaser(struct pppLaser *pppLaser, struct pppLaserUnkB *param_2, _pppCtrlTable *param_3)
{
    ...
    GXColor1u32(*(u32*)&color);
    GXTexCoord2f32(LaserConst(FLOAT_8033342c), LaserConst(kPppLaserZero));
    GXPosition3f32(halfWidth, LaserConst(kPppLaserZero), length);
    GXColor1u32(*(u32*)&color);
    GXTexCoord2f32(LaserConst(FLOAT_8033342c), work->m_length);
    ...
    if (step->m_stepValue != 0xFFFF) {
        ...
        uvStep = LaserConst(FLOAT_8033342c) / (float)count;
        ...
        for (int j = 0; j < (int)(step->m_laser.m_pointCount - 1); j++) {
            ...
            GXTexCoord2f32(u0, LaserConst(FLOAT_8033342c));
        ...
        if ((CFlatRuntimeDebugFlags() & CFlatRuntimeDebugFlag_ParticleHitSpheres) != 0) {
            ...
            if ((CFlatRuntimeDebugFlags() & CFlatRuntimeDebugFlag_ParticleHitSpheres) != 0) {
                ...
                debugSource.z = LaserConst(FLOAT_8033342c);
                color.r = 0xFF;
                color.g = 0xFF;
                color.b = 0xFF;
                color.a = 0xFF;
            ...
            for (i = 0; i < (int)(u32)step->m_laser.m_pointCount; i++) {
                ...
                PSMTXScale(pointMtx, LaserConst(FLOAT_80333430), LaserConst(FLOAT_80333430), LaserConst(FLOAT_80333430));
