...
undefined4 setSize__22CPtrArray<P9CMaterial>FUl(CPtrArray *ptrArray,uint size)
  ...
  if (ptrArray->size < size) {
    if (ptrArray->size == 0) {
      ...
      if (ptrArray->growCapacity == 0) {
        Printf__7CSystemFPce(&System,&DAT_801d7a84);
      }
    ...
    dstBuffer = (void **)_Alloc__7CMemoryFUlPQ27CMemory6CStagePcii
                                   (&Memory,ptrArray->size << 2,ptrArray->stage,
                                    s_collection_ptrarray_h_801d7aa0,0xfa,0);
