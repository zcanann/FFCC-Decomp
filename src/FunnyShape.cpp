...
static inline FS_Animation* AnimData(CFunnyShape* self)
{
    return static_cast<FS_Animation*>(self->m_anm.anmData);
}
...
void CFunnyShape::RenderShape(FS_tagOAN3_SHAPE* shape, Vec2d offset, float angle)
{
    ...
    for (s32 i = 0; i < *reinterpret_cast<const s16*>(shapeData + 2); i++) {
        ...
        if ((*reinterpret_cast<const s16*>(shapeData) & 8) != 0) {
            ...
            GXSetViewport(viewportScale * min.x + offset.x, viewportScale * min.y + offset.y, viewportW,
...
void CFunnyShape::ClearTextureData()
{
    ...
    for (s32 i = 0; i < 0x10; i++) {
        if (m_textureData[i] != 0) {
            delete[] static_cast<u8*>(m_textureData[i]);
            m_textureData[i] = 0;
        }

        if (m_texObjData[i] != 0) {
            delete m_texObjData[i];
            m_texObjData[i] = 0;
        }

        if (m_textureHeaders[i] != 0) {
            delete m_textureHeaders[i];
...
void CFunnyShape::ClearAnmData()
{
    ...
    memset(m_anmWork, 0, sizeof(CFunnyShapeAnmWork));
...
void CFunnyShape::RenderShape()
{
    ...
    GXLoadTexObj(m_texObjData[0], GX_TEXMAP0);
    GXSetNumChans(1);
...
void CFunnyShape::Render()
{
    ...
    for (s32 i = 0; i < count; i++) {
        ...
        FS_Animation* animData = AnimData(this);
        const FS_Animation::Frame* frameData = reinterpret_cast<const FS_Animation::Frame*>(animData + 1);
        s16 frame = work->frame;
        frameData += frame;
        FS_tagOAN3_SHAPE* shape = reinterpret_cast<FS_tagOAN3_SHAPE*>(
            reinterpret_cast<u8*>(animData) + frameData->shapeOffset);
...
void CFunnyShape::Update()
{
    ...
    for (s32 i = 0; i < ShapeCount(this); i++) {
        ...
        if (work->delay <= 0) {
            ...
            if (work->frame >= AnimData(this)->frameCount) {
                work->frame = 0;
            ...
            const FS_Animation::Frame* frameData = reinterpret_cast<const FS_Animation::Frame*>(AnimData(this) + 1);
            s16 frame = work->frame;
            frameData += frame;
            work->delay = frameData->delay;
        }
...
{
    m_meshData = 0;
    memset(m_anmWork, 0, sizeof(m_anmWork));
    memset(&m_anm, 0, sizeof(OSFS_ANM_ST));
