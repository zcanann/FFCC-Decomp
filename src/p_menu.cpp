...
void CMenuPcs::changeMode(CMenuPcs::MENUMODE mode)
{
    ...
    if (*reinterpret_cast<int*>(reinterpret_cast<u8*>(this) + 0x740) != static_cast<int>(mode)) {
        _WaitDrawDone__8CGraphicFPci(&Graphic, const_cast<char*>(s_p_menu_cpp_801d9d80), 0x1B0);
...
void CMenuPcs::SetExtraFontTlut(int fontNo, _GXColor color)
{
    ...
    for (int i = 0; i < 0x10; i++) {
        CTexture* texture =
            *reinterpret_cast<CTexture**>(*reinterpret_cast<u32*>(self + 0x100 + slotOffset) + 0x34);
        u8 alpha = texture->GetTlutColor(i).a;
        _GXColor out;
...
{
    static const int kBattleTextureSetStart = 2;
    static const int kBattleTextureStart = 0x16;
...
void CMenuPcs::drawBattle()
{
    if (m_battleHud.m_visible != 0) {
        ...
        if (projected.w > LoadFloat(kMenuInitOne)) {
            ...
            if (screenX < static_cast<float>(halfWidth)) {
                ...
                const float right = LoadFloat(kMenuOrthoRight) - static_cast<float>(halfWidth);
                if (screenX > right) {
                    screenX = right;
                }
            }
