// Function: setSize__38CPtrArray<PQ29CCharaPcs12CLoadTexture>FUl
// Entry: 8007adb8
// Size: 240 bytes

undefined4 setSize__38CPtrArray<PQ29CCharaPcs12CLoadTexture>FUl(CPtrArray *ptrArray,uint size)
  ...
  if (ptrArray->size < size) {
    if (ptrArray->size == 0) {
      ...
      if (ptrArray->growCapacity == 0) {
        Printf__7CSystemFPce(&System,&DAT_801d9aa8);
      }
    ...
    dstBuffer = (void **)_Alloc__7CMemoryFUlPQ27CMemory6CStagePcii
                                   (&Memory,ptrArray->size << 2,ptrArray->stage,
                                    s_collection_ptrarray_h_801d9ac4,0xfa,0);
