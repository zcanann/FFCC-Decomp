...
static const char s_dvd__smenu_subfont__s_fnt_801dc46c[] = "dvd/smenu/subfont_%s.fnt";
static const char s_dvd__smenu_option__s_tex_801dc484[] = "dvd/smenu/option_%s.tex";
...
static const unsigned int s_wmFrameInitTable[24] = {
    0x03200000, 0x00000000, 0x00000000, 0x00000080, 0x000000A0, 0x00000080,
    0x00000000, 0x00000078, 0x00000040, 0x00000260, 0x000000F8, 0x00000020,
    0x00000040, 0x00000238, 0x00000138, 0x00000040, 0x00000038, 0x000001A0,
    0x00000170, 0x000000C8, 0x00000050, 0x00000000, 0x00000000, 0x43000000,
};
...
void CMenuPcs::loadData()
{
	...
	*reinterpret_cast<float*>(reinterpret_cast<unsigned char*>(m_wm.m_handles[6]->m_model) + 0x9C) =
	...
	{
		unsigned int frameTemplate[20];
		for (int i = 0; i < 20; i++) {
			frameTemplate[i] = s_wmFrameInitTable[i + 1];
		}
		const unsigned int* src = frameTemplate;
		unsigned char* dst = m_wm.m_frameData + 0xC;
		const float one = FLOAT_803313e8;
		for (int i = 0; i < 5; i++) {
			*reinterpret_cast<short*>(dst + 0x00) = static_cast<short>(src[0]);
			*reinterpret_cast<short*>(dst + 0x02) = static_cast<short>(src[1]);
			*reinterpret_cast<short*>(dst + 0x04) = static_cast<short>(src[2]);
			*reinterpret_cast<short*>(dst + 0x06) = static_cast<short>(src[3]);
			*reinterpret_cast<unsigned int*>(dst + 0x08) = src[0];
			*reinterpret_cast<unsigned int*>(dst + 0x0C) = src[1];
			*reinterpret_cast<float*>(dst + 0x10) = one;
			*reinterpret_cast<float*>(dst + 0x14) = one;
			*reinterpret_cast<unsigned int*>(dst + 0x18) = 0;
			dst += 0x1C;
			src += 4;
		}
	}
	...
	memset(m_wm.m_frameInfo, 0, kWmFrameInfoBytes);
	{
		...
		*reinterpret_cast<short*>(frame + 0x20) =
		    0x280 - (*reinterpret_cast<short*>(frame + 8) + *reinterpret_cast<short*>(frame + 4));
	...
	bytes[0x86E] = 0;
	bytes[0x858] = 0;
	*reinterpret_cast<unsigned int*>(bytes + 0x854) = 0;
	...
	float defaultScale = FLOAT_803313e8;
	...
	for (int i = 0; i < 8; i++, animOff += 0x14) {
		...
		if (handle->m_charaKind != 3) {
			...
			*reinterpret_cast<int*>(reinterpret_cast<unsigned char*>(m_wmCharaAnimState) + animOff + 0x10) =
			...
			float endFrame =
			    static_cast<float>(static_cast<double>(reinterpret_cast<unsigned short*>(animData + 0x10)[0]));
			if (FLOAT_8032ee18 < endFrame) {
				FLOAT_8032ee18 = endFrame;
			}
		}
	}

	{
		CCharaPcs::CHandle* const world = reinterpret_cast<CCharaPcs::CHandle**>(bytes + 0x778)[0];
		world->LoadAnim(lbl_80331800, 0, 2, -1, -1, -1, 0);
		world->LoadAnim(lbl_80331808, 1, 0, -1, -1, -1, 0);
		world->LoadAnim(lbl_80331810, 2, 2, -1, -1, -1, 0);
		world->LoadAnim(lbl_80331818, 3, 2, -1, -1, -1, 0);
		world->LoadAnim(lbl_80331820, 4, 2, -1, -1, -1, 0);
		world->LoadAnim(lbl_80331828, 5, 2, -1, -1, -1, 0);
		world->LoadAnim(lbl_80331830, 6, 2, -1, -1, -1, 0);
		world->LoadAnim(lbl_80331838, 7, 0, -1, -1, -1, 0);
		world->SetAnim(0, -1, -1, -1, 0);
	}

	unsigned char* const worldState = reinterpret_cast<unsigned char*>(m_wmWorldState);
	*reinterpret_cast<unsigned short*>(worldState + 0x1C) = 0;
	...
	*reinterpret_cast<short*>(worldState + 0x44) = static_cast<short>(Game.m_gameWork.m_wmBackupParams[3]);
	...
	char fontPath[128];
	sprintf(fontPath, const_cast<char*>(s_dvd__smenu_subfont__s_fnt_801dc46c), Game.GetLangString());
	...
	bytes[0x12] = 0;
	bytes[0x13] = 0;
	*reinterpret_cast<unsigned short*>(worldState + 0x20) = 0;
	*reinterpret_cast<unsigned short*>(worldState + 0x1E) = 0;
	*reinterpret_cast<unsigned short*>(worldState + 0x18) = 0;
	DAT_8032ee1c = 1;
	DAT_8032e8ac = 1;

	for (int i = 4; i < 6; i++) {
		CMesMenu* menu = new (m_menuStage, const_cast<char*>(s_wm_menu_cpp), 0x2EA) CMesMenu;
	...
	char texPath[256];
	sprintf(texPath, const_cast<char*>(s_dvd__smenu_option__s_tex_801dc484), Game.GetLangString());
	CFile::CHandle* fileHandle = File.Open(texPath, 0, CFile::PRI_LOW);
	if (fileHandle != 0) {
		File.Read(fileHandle);
		File.SyncCompleted(fileHandle);
		CTextureSet* textureSet = new (m_menuStage, const_cast<char*>(s_wm_menu_cpp), 0x300) CTextureSet;
		reinterpret_cast<CTextureSet**>(bytes + 0xBC)[0] = textureSet;
		reinterpret_cast<CTextureSet**>(bytes + 0xBC)[0]->Create(File.m_readBuffer, m_menuStage, 0, 0, 0, 0);
		File.Close(fileHandle);
	}

	CTexture** textureSlot = reinterpret_cast<CTexture**>(bytes + 0xC0);
	for (unsigned int i = 0;
	     i < static_cast<unsigned int>(reinterpret_cast<CTextureSet**>(bytes + 0xBC)[0]->m_textureArray.GetSize());
	     i++) {
		textureSlot[i] = reinterpret_cast<CTextureSet**>(bytes + 0xBC)[0]->m_textureArray[i];
	}

	GetOptionData();
	bytes[0x80] = 0;
	*reinterpret_cast<float*>(bytes + 0x7C) = FLOAT_803313dc;
	*reinterpret_cast<float*>(bytes + 0x78) = FLOAT_803313dc;
	*reinterpret_cast<unsigned short*>(bytes + 0x74) = 0;
	DAT_8032ef08 = reinterpret_cast<int>(&g_GoOutMenu);
}
...
void CMenuPcs::CalcMCardMenu()
{
	...
	*reinterpret_cast<short*>(iVar12 + 0x20) = (short)iVar14;
	...
	if ((int)uVar15 < 0) {
		double dVar22 = (double)(float)((int)*reinterpret_cast<short*>(frame + 8) + (int)*reinterpret_cast<short*>(frame + 4));
		if ((int)uVar15 > -11) {
			unsigned int uVar11 = (int)uVar15 >> 0x1f;
			double dVar23 = (double)(float)(dVar22 * DOUBLE_803314e8 * static_cast<double>((int)((uVar11 ^ uVar15) - uVar11)));
			unsigned int absOff = (uVar11 ^ uVar15) - uVar11;
			if ((int)absOff < 0) absOff = 0;
			if ((int)absOff > 10) absOff = 10;
			dVar22 = (double)(float)sin((double)(FLOAT_803314bc * (float)(int)absOff * FLOAT_803316d4));
			dVar22 = (double)(float)(dVar23 * (double)(float)dVar22);
		}

		iVar14 = (int)((double)(float)(int)*reinterpret_cast<short*>(frame + 4) - dVar22);
		*reinterpret_cast<short*>(frame + 4) = (short)iVar14;
		iVar14 = (int)((double)(float)(int)*reinterpret_cast<short*>(frame + 0x20) + dVar22);
	...
	case 2:
		if (m_wmWorldState->m_flag09 == 0) {
			m_wmWorldState->m_cardChannel = static_cast<short>(mcCtrl.m_cardChannel);
			...
			if (m_wmWorldState->m_counter1A == 0) {
				if ((uVar6 & 3) == 0) {
					if ((uVar3 & 0x100) == 0) {
						...
						m_wmWorldState->m_state0E = 1;
						mcCtrl.m_cardChannel = m_wmWorldState->m_cardChannel;
						if (m_wmWorldState->m_cardChannel == gWmMenuCursorX[0]) {
							mcCtrl.m_saveIndex = (int)gWmMenuCursorX[1];
						}
		...
		break;
	case 3:
		m_wmWorldState->m_mcResult = static_cast<short>(MemoryCardMan.McChkConnect(mcCtrl.m_cardChannel));
		break;
	case 4:
		if (m_wmWorldState->m_flag09 == 0) {
			CFont* pFont = m_fonts[0];
	...
	case 0x1C: {
		...
		if (m_menuWindowInfo->state == 1
			...
			LAB_chk:
				...
				} else if (m_wmWorldState->m_mcResult != sVar16 && m_wmWorldState->m_mcResult != 1) {
					m_wmWorldState->m_state0E = -1;
					m_wmWorldState->m_counter1A = 1;
					break;
				}
	...
	case 0xC: {
		...
		if (m_menuWindowInfo->state == 1
		    && m_wmWorldState->m_counter1A == 0) {
			mcCtrl.m_lastResult = mcCtrl.LoadMcList();
	...
	case 0x1A: {
		if (m_wmWorldState->m_flag09 == 0) {
			...
			if (iVar12 == 0xD) {
				mcCtrl.Format(1);
				m_wmWorldState->m_mcResult = (short)mcCtrl.m_lastResult;
				if (m_wmWorldState->m_mcResult < 0) {
					MemoryCardMan.m_opDoneFlag = 1;
					MemoryCardMan.m_currentSlot = 0xFF;
				}
			} else if (iVar12 == 0x1A) {
				mcCtrl.m_lastResult = mcCtrl.EraseDat();
				m_wmWorldState->m_mcResult = (short)mcCtrl.m_lastResult;
				if (m_wmWorldState->m_mcResult < 0) {
					MemoryCardMan.m_opDoneFlag = 1;
					MemoryCardMan.m_currentSlot = 0xFF;
				...
				mcCtrl.m_lastResult = mcCtrl.SaveDat();
				m_wmWorldState->m_mcResult = (short)mcCtrl.m_lastResult;
				if (m_wmWorldState->m_mcResult < 0) {
					MemoryCardMan.m_opDoneFlag = 1;
					MemoryCardMan.m_currentSlot = 0xFF;
		...
		if (m_wmWorldState->m_mcResult == 0 && uVar6 != 0) {
			...
			if ((uVar6 & 0xC) == 0) {
				if ((uVar3 & 0x100) != 0) {
					...
					if (sVar7 != -1 && sVar7 != 1) {
						mcCtrl.m_saveIndex = (int)m_wmWorldState->m_cardChannel;
						m_wmWorldState->m_state0E = 1;
						Sound.PlaySe(2, 0x40, 0x7F, 0);
					}
				}
...
void CMenuPcs::CalcLoadMenu()
{
	...
	if (static_cast<signed char>(m_wmWorldState->m_worldReady) == 0) {
		...
		*reinterpret_cast<short*>(m_wm.m_frameInfo + 6) = 0x10;
		*reinterpret_cast<short*>(m_wm.m_frameInfo + 8) = 0xE8;
		*reinterpret_cast<short*>(m_wm.m_frameInfo + 10) = 0x168;
		*reinterpret_cast<float*>(m_wm.m_frameInfo + 0xC) = fVar4;
		*reinterpret_cast<float*>(m_wm.m_frameInfo + 0x10) = fVar4;
		*reinterpret_cast<float*>(m_wm.m_frameInfo + 0x14) = fVar5;
		*reinterpret_cast<float*>(m_wm.m_frameInfo + 0x18) = fVar5;
		...
		*reinterpret_cast<float*>(iVar14 + 0x28) = *reinterpret_cast<float*>(iVar14 + 0xC);
		*reinterpret_cast<float*>(iVar14 + 0x2C) = *reinterpret_cast<float*>(iVar14 + 0x10);
		*reinterpret_cast<float*>(iVar14 + 0x30) = *reinterpret_cast<float*>(iVar14 + 0x14);
		*reinterpret_cast<float*>(iVar14 + 0x34) = *reinterpret_cast<float*>(iVar14 + 0x18);
			...
			if (m_wmWorldState->m_counter1A == 0) {
				if ((uVar7 & 3) == 0) {
					if ((uVar4 & 0x100) == 0) {
						...
						m_wmWorldState->m_state0E = 1;
						mcCtrl.m_cardChannel = (int)m_wmWorldState->m_cardChannel;
						m_wmWorldState->m_counter1A = 10;
						Sound.PlaySe(2, 0x40, 0x7F, 0);
					}
				} else {
	...
	case 0x1A: {
		if (static_cast<signed char>(m_wmWorldState->m_flag09) == 0) {
			...
			if (iVar10 == 0xD) {
				...
				mcCtrl.m_lastResult = mcCtrl.LoadDat();
	...
	if (m_wmWorldState->m_mainState <= 4) {
		...
		*reinterpret_cast<short*>(m_wm.m_frameInfo + 0x20) =
		...
		if (selectState[0x0E] == 0) {
			...
			if (selectState[0x0A] == 0 ||
			    (*reinterpret_cast<int*>(reinterpret_cast<unsigned char*>(m_wmCharaAnimState) + cursor * 0x14) == 3 && slot != 0)) {
				result = -1;
			} else {
				result = cursor;
			}
...
void CMenuPcs::drawWorld()
{
	...
	if (bytes[0xD] == 0) {
		for (int i = 4; i < 6; i++) {
			CMenu* const menu = reinterpret_cast<CMenu**>(bytes + 0x11C)[i - 4];
			menu->Draw();
		}
	} else {
...
void CMenuPcs::DrawMainMenu()
{
	...
	if (frame != 0) {
		...
		MenuPcs.DrawRect(*reinterpret_cast<unsigned int*>(entry + 0x18), static_cast<float>(*reinterpret_cast<short*>(entry + 0)),
	...
	if (m_wmWorldState->m_mainState > 0 && m_wmWorldState->m_mainState < 4) {
		...
		MenuPcs.DrawRect(8, static_cast<float>(static_cast<double>(x) + static_cast<double>(FLOAT_803316d0)), y,
		...
		MenuPcs.DrawRect(4, x, y, FLOAT_803316d0, FLOAT_80331500, FLOAT_803313dc, FLOAT_803313dc, FLOAT_803313e8,
		         FLOAT_803313e8, FLOAT_803313dc);
		MenuPcs.DrawRect(0xc, static_cast<float>(static_cast<double>(x) + static_cast<double>(FLOAT_803316d0)), y,
		         FLOAT_803316d0, FLOAT_80331500, FLOAT_803313dc, FLOAT_803313dc, FLOAT_803313e8, FLOAT_803313e8, FLOAT_803313dc);
	}
...
void CMenuPcs::DrawDiaryMenu()
{
	...
	if (handle != 0) {
		...
		Mtx lookAtMtx;
		C_MTXLookAt(lookAtMtx, reinterpret_cast<Vec*>(worldObj + 0x60), reinterpret_cast<Vec*>(&up), reinterpret_cast<Point3d*>(&target));
...
void CMenuPcs::DrawMCardMenu()
{
	...
	if (state == 2 && m_wmWorldState->m_subState > 0x10) {
		float cursorX = static_cast<float>(static_cast<double>(FLOAT_803314d8) - DOUBLE_803317D8);
		...
		float cursorY = static_cast<float>(DOUBLE_80331498 * static_cast<double>(saveIdx) +
		         static_cast<double>(static_cast<float>(static_cast<double>(FLOAT_803314d8) + static_cast<double>(FLOAT_80331410))));
		MenuPcs.SetAttrFmt((FMT)0);
		unsigned int cursorColor = 0xFFFFFFFF;
		GXSetChanMatColor(GX_COLOR0A0, *(_GXColor*)&cursorColor);
		MenuPcs.SetTexture((TEX)0);
		MenuPcs.DrawRect(0xFFFFFFFF, static_cast<float>(static_cast<int>(cursorX)), static_cast<float>(static_cast<int>(cursorY)),
	...
	if (state == 2 && m_wmWorldState->m_delay == 0) {
		...
		case 2:
			...
			if (winState == 1) {
				DrawMcWinMess(0, 0);
				float rectY = static_cast<float>(static_cast<double>(static_cast<int>(
				         m_menuWindowInfo->y + m_menuWindowInfo->height - 0x3e)) - DOUBLE_80331408);
				int slotX = GetSlotABXPos(m_wmWorldState->m_cardChannel);
				float rectX = static_cast<float>(static_cast<double>(static_cast<int>(slotX)) - DOUBLE_80331408);
				MenuPcs.SetAttrFmt((FMT)0);
				unsigned int abxColor = 0xFFFFFFFF;
				GXSetChanMatColor(GX_COLOR0A0, *(_GXColor*)&abxColor);
				MenuPcs.SetTexture((TEX)0);
				MenuPcs.DrawRect(0xFFFFFFFF, static_cast<float>(static_cast<int>(rectX)),
				         static_cast<float>(static_cast<int>(rectY)),
				         FLOAT_80331410, FLOAT_80331410,
				         FLOAT_803313dc, FLOAT_803313dc,
				         FLOAT_803313e8, FLOAT_803313e8, 0);
			}
		...
		case 4:
			...
			if (winState == 1) {
				...
				if (mcRes != 0 && mcRes == 1) {
					m_wmWorldState->m_subState = 0x0C;
				}
		...
		case 0x1C:
			...
			if (winState == 2 && m_menuWindowInfo->state == 3) {
				short ss;
				if (m_wmWorldState->m_state0E < 0 ||
				    ((ss = m_wmWorldState->m_subState, ss == 0x14 || ss == 0x1C) || ss == 0x1B)) {
					m_wmWorldState->m_subState = 3;
				} else if (ss == 0x0E) {
					memset(GetWmMenuCharaState(this), 0, kWmMenuCharaStateBytes);
		...
		case 0x19:
			...
			if (winState == 1) {
				...
				float rectY = static_cast<float>(static_cast<double>(static_cast<int>(
				         m_menuWindowInfo->y + m_menuWindowInfo->height - 0x3e)) - DOUBLE_80331408);
				int ynX = GetYesNoXPos(m_wmWorldState->m_cardChannel);
				float rectX = static_cast<float>(static_cast<double>(static_cast<int>(ynX)) - DOUBLE_80331408);
				if (m_wmWorldState->m_subState == 0x19) {
					short mcRes = m_wmWorldState->m_mcResult;
					if (mcRes < 0) {
						if (mcRes == -1) m_wmWorldState->m_subState = 5;
						else if (mcRes == -2) m_wmWorldState->m_subState = 8;
						else if (mcRes == -3) m_wmWorldState->m_subState = 6;
		...
		case 0x0C:
			...
			if (((winState != 1 ||
				...
				if (cRes == -1) m_wmWorldState->m_subState = 10;
					...
					BindMcObj();
					unsigned char* charaState = GetWmMenuCharaState(this);
					unsigned int slot = 0;
					if (charaState[0x42] == '\0' &&
					    (slot = 1, charaState[0x8a] == '\0') &&
					    (slot = 2, charaState[0xd2] == '\0' &&
					     (slot = 3, charaState[0x11a] == '\0'))) {
						slot = 4;
					}
					if (slot < 4) {
		...
		case 0x11:
			if (m_wmWorldState->m_mcResult < 0) {
				...
				} else if (GetWmMenuCharaState(this)[mcCtrl.m_saveIndex * 0x48 + 0x41] == '\0') {
					m_wmWorldState->m_subState = 0x13;
				} else {
...
void CMenuPcs::DrawCMakeMenu()
{
	...
	if (state > 0 && state < 4) {
		...
		DrawRect(0, FLOAT_803313dc, static_cast<float>(DOUBLE_803314d0 - static_cast<double>(FLOAT_80331440)),
		         FLOAT_803313e0, FLOAT_80331440, FLOAT_803313dc, FLOAT_803313dc, FLOAT_803313e8, FLOAT_803313e8, FLOAT_803313dc);
...
void CMenuPcs::DrawMoveMenu()
{
	...
	if (m_wmWorldState->m_mainState > 0 && m_wmWorldState->m_mainState < 3) {
		...
		GXSetProjection(projectionMtx, GX_PERSPECTIVE);
		PSMTX44Copy(projectionMtx, CameraPcs.m_screenMatrix);
		C_MTXLookAt(lookAtMtx, reinterpret_cast<Point3d*>(worldObj + 0x1A0), reinterpret_cast<Vec*>(&up), reinterpret_cast<Point3d*>(&target));
		PSMTXCopy(CameraPcs.m_cameraMatrix, m_wm.m_cameraMatrix);
		PSMTXCopy(lookAtMtx, CameraPcs.m_cameraMatrix);
		CharaPcs.InitEnv(5);
		GXSetColorUpdate(0);
...
{
	unsigned char* const bytes = reinterpret_cast<unsigned char*>(this);
	short sVar3 = m_wmWorldState->m_mainState;
	if (sVar3 == 0) {
		return;
	}
	if (sVar3 > 2) {
...
void CMenuPcs::PCAnimCtrl()
{
	...
	for (int i = 0; i < kWmMenuPlayerCount; i++, animState += 5) {
		CCharaPcs::CHandle* const handle = GetWmCharaHandles(this)[i];
		const int blendMode = -1 - static_cast<int>(static_cast<unsigned int>(handle->m_currentAnimIndex) >> 31);
...
void CMenuPcs::DrawMainMenuSub()
{
	...
	for (int orderIndex = 0; orderIndex < 5; orderIndex++) {
		...
		if ((state < 1 || state > 3) && (worldState->m_cardChannel != 1 || orderIndex != 1)) {
			continue;
		}
		if ((state < 1 || state > 3) && worldState->m_cardChannel == 1 && orderIndex == 1) {
			handleIndex = 1;
...
LAB_draw:
	...
	if (worldState->m_subState > 0x10 &&
		...
		for (int slot = 0; slot < 4; slot++) {
			...
			short sub2 = worldState->m_subState;
			if (sub2 == 1 || worldState->m_mainState == 3) {
				int af2;
				if (worldState->m_mainState == 2) {
					af2 = (int)worldState->m_frameCounter - slot * 3;
				} else {
					af2 = 10 - ((int)worldState->m_frameCounter + (3 - slot) * -3);
				}
				if (af2 < 0) continue;
				if (af2 < 11) {
					alpha2 = (float)af2;
...
void CMenuPcs::DrawRect2(unsigned long flags, float x, float y, float w, float h, float tx, float ty, float scaleX, float scaleY, float (*mtx)[4])
{
	...
	for (int i = 0; i < 4; i++) {
		...
		GXTexCoord2f32((i & 1) != 0 ? u1 : u0, i < 2 ? v0 : v1);
	}
}
...
int CMenuPcs::GetSameCharaData(Mc::SaveDat* source, Mc::SaveDat* target, int memberIndex, int strictMode)
{
	...
	if (strictMode == 0) {
		...
		if (*reinterpret_cast<unsigned long long*>(src + 0x13D0) != *reinterpret_cast<unsigned long long*>(targetHeader + 0x1D98) ||
				...
				if (src[0x1D90] != 0 &&
				    *reinterpret_cast<unsigned int*>(src + 0x1D94) == *reinterpret_cast<unsigned int*>(dst + cmpOffset)) {
					break;
			           ...
			           *reinterpret_cast<unsigned int*>(src + 0x1D94) == *reinterpret_cast<unsigned int*>(dst + cmpOffset) &&
			           *reinterpret_cast<unsigned long long*>(src + 0x1D98) == *reinterpret_cast<unsigned long long*>(dst + 0x13D0) &&
				...
				if (src[0x2750] != 0 &&
				    *reinterpret_cast<unsigned int*>(src + 0x2754) == *reinterpret_cast<unsigned int*>(dst + cmpOffset)) {
					break;
			           ...
			           *reinterpret_cast<unsigned int*>(src + 0x2754) == *reinterpret_cast<unsigned int*>(dst + cmpOffset) &&
			           *reinterpret_cast<unsigned long long*>(src + 0x2758) == *reinterpret_cast<unsigned long long*>(dst + 0x13D0) &&
	...
	if (*reinterpret_cast<char*>(save + 0x10C0) != 0) {
		...
		if (crcOk == 1 && formatMatch == 0) {
			...
			*reinterpret_cast<unsigned int*>(entry + 0x14) = *reinterpret_cast<unsigned int*>(save + 0x2C);
			...
			unsigned char* slot = save + *reinterpret_cast<int*>(save + 0x30) * 0x9C0;
			if (*reinterpret_cast<int*>(slot + 0x1A84) == 0 || slot[0x1D90] != 0) {
				*reinterpret_cast<int*>(save + 0x30) = -1;
			}
			slot = save + *reinterpret_cast<int*>(save + 0x34) * 0x9C0;
			if (*reinterpret_cast<int*>(slot + 0x1A84) == 0 || slot[0x1D90] != 0) {
				*reinterpret_cast<int*>(save + 0x34) = -1;
			}
			slot = save + *reinterpret_cast<int*>(save + 0x38) * 0x9C0;
			if (*reinterpret_cast<int*>(slot + 0x1A84) == 0 || slot[0x1D90] != 0) {
				*reinterpret_cast<int*>(save + 0x38) = -1;
			}
			slot = save + *reinterpret_cast<int*>(save + 0x3C) * 0x9C0;
			if (*reinterpret_cast<int*>(slot + 0x1A84) == 0 || slot[0x1D90] != 0) {
