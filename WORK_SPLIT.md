...
- **SESSION SUMMARY (Opus structural campaign, B1 95.85->96.02 avg, ~45 function wins):** the user's "low-90s% means structural bugs, not just register walls" was correct for a LARGE batch of functions. Full lever catalog discovered/validated this campaign (try ALL of these before declaring a register wall): segW conversion-hoist; modulo-as-subtraction; redundant-complement-if; volatile-loop-start; scoped opt_propagation off; branch-layout (if(cond){err;return}succ -> if(!cond){succ}else{...}); adjacent-buffer-read (*(&x+1)); operand-eval-order via named temp; non-const-int-to-force-li; literal-not-field const arg; subexpr-reuse; int->double conversion statement-order; named-pointer const-fold-prevention (only when it preserves an @sda21 load); index-materialization (hoist arr[expr] subscript to a named int idx); store-order (store the zero/constant operand first to claim the lower FP reg); pointer-route-for-eval-order (route a field load through an already-materialized pointer); scaleMtx-store-reorder. EXHAUSTION STATE after this campaign: every B1 unit had 4-6 independent Opus passes + permuter + the full toolkit; the RESIDUAL is now genuinely the MWCC-version floor — whole-function register-numbering cascades from a single early seam, saved-register RANKING inversions (CalcResultOpenAnim tribeId r30/r31, ChkUnite caravan/selected), frame-size/spill-strategy deltas (DrawMCList, DrawCMakeMenu, CalcResultOpenAnim +0x40), GXColor param-home stack-offset, and FP-register numbering — none reachable from plausible C++ with this toolchain. Further gains need new lever discovery, deep per-function reconstruction, or maintainer-level toolchain/objdiff-attribution changes.
...
- **named-pointer anti-folding lever (NEW, +0.11 on wm DrawMainMenuSub — @NNN is NOT always cosmetic):** referencing an inline float/double constant via `const float* p = &FLOAT_x; ... *p` (instead of the bare `FLOAT_x`) forces MWCC to emit a NAMED `FLOAT_/DOUBLE_@sda21` reloc instead of interning the value into an anonymous `@NNN` pool slot. REFINED (wm DrawMoveMenu +1.2 / DrawDiaryMenu +2.6): the named-pointer lever HAS TWO CASES. (A) RELOC-NAMING case = NO-OP: a bare `FLOAT_x` load that the target also loads — naming it changes only @NNN-vs-named (value-keyed, doesn't score). (B) CONST-FOLD-PREVENTION case = REAL WIN: an arithmetic expression like `DOUBLE_x - FLOAT_y` (both with visible initializers) gets CONSTANT-FOLDED by MWCC into a single anonymous `@NNN` pool slot, but the TARGET computes it at RUNTIME (`lfd`/`lfs` + `fsub` + `frsp`). Wrapping each operand in `const T* p = &SYM; *p` defeats the fold and emits the runtime instruction sequence — this is a REAL instruction-level match (+1-2.6pp). So: use named-pointer when the diff shows OURS has a folded constant where TARGET has a runtime fsub/fadd/fmul — NOT for bare reloc-naming.
CORRECTION (confirmed on wm round-3): report.json's fuzzy scorer treats `@NNN@sda21` and `named@sda21` as EQUIVALENT (same reloc type + target value) — converting @NNN->named is a NO-OP on report.json (verified byte-identical on DrawTitleMenu/CalcLoadMenu). DrawMainMenuSub's small gain came from the `*p` deref incidentally changing instruction SCHEDULING, not the naming. So: do NOT chase @NNN-vs-named for the reloc naming itself (cosmetic on report.json, matching the original calibration); only useful as a scheduling perturbation, which is unpredictable — gate per-edit and don't rely on it.
