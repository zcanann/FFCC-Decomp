...
void CMenuPcs::FavoDraw()
{
	...
	for (int i = 0; i < 8; i++) {
		...
		DrawSingBar(barX, barY, rank->value, drawEntry->alpha);
		rank++;
		drawEntry++;
	}
	...
	for (int i = 0; i < 8; i++) {
		...
		int iconY = static_cast<int>((static_cast<float>(drawEntry->h) - FLOAT_80333044) * DOUBLE_80333060 +
		                             static_cast<float>(drawEntry->y));
		DrawSingleIcon(rank->food + 0x14, iconX, iconY, drawEntry->alpha, 1, FLOAT_80333048);
		rank++;
		drawEntry++;
	}
	...
	for (int i = 0; i < 8; i++) {
		...
		sprintf(textBuf, s_FavoRankFormat_80333068, static_cast<int>(rank->rank));
	...
	for (int i = 0; i < 8; i++) {
		nameFont->SetColor(
		    CColor(0xFF, 0xFF, 0xFF, static_cast<unsigned char>(FLOAT_80333058 * drawEntry->alpha)).color);
		const char* name = Game.m_cFlatDataArr[1].TableStrings(0)[(rank->food + 0x17D) * 5 + 4];
