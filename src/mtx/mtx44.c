...
void C_MTX44TransApply(const Mtx44 src, Mtx44 dst, f32 xT, f32 yT, f32 zT) {
    ...
    ASSERTMSGLINE(900, dst, "MTX44TransApply(): NULL Mtx44Ptr 'src' ");
...
void PSMTX44RotTrig(register Mtx44 m, register char axis, register f32 sinA, register f32 cosA) {
    ...
    asm {
        ...
        frsp cosA, cosA
        cmplwi axis, 'x'
        beq L_PSMTX44RotTrig_x
        cmplwi axis, 'y'
        beq L_PSMTX44RotTrig_y
        cmplwi axis, 'z'
        beq L_PSMTX44RotTrig_z
        b L_PSMTX44RotTrig_end
    L_PSMTX44RotTrig_x:
        psq_st c_one, 0x0(m), 1, 0
        psq_st c_zero, 0x4(m), 0, 0
        ps_neg ftmp0, sinA
