...
void CRedEntry::Init()
{
	...
	iVar2 = 0;
	do {
		*(int*)(m_waveBankBase + iVar2 * REDSOUND_HISTORY_BANK_ENTRY_SIZE) = REDSOUND_HISTORY_BANK_EMPTY_ID;
		iVar2 = iVar2 + 1;
	...
	iVar2 = 0;
	do {
		*(int*)(m_seSepBankBase + iVar2 * REDSOUND_HISTORY_BANK_ENTRY_SIZE) = REDSOUND_HISTORY_BANK_EMPTY_ID;
		iVar2 = iVar2 + 1;
	...
	iVar2 = 0;
	do {
		*(int*)(m_musicBankBase + iVar2 * REDSOUND_HISTORY_BANK_ENTRY_SIZE) = REDSOUND_HISTORY_BANK_EMPTY_ID;
		iVar2 = iVar2 + 1;
...
{
	RedHistoryBANK* history = reinterpret_cast<RedHistoryBANK*>(m_waveBankBase + REDSOUND_WAVE_HISTORY_BANK_OFFSET);
...
int CRedEntry::SearchWaveSequence(int waveNo)
{
	...
	while (waveBank < reinterpret_cast<RedHistoryBANK*>(m_waveBankBase + REDSOUND_WAVE_BANK_SIZE)) {
		if ((waveBank->m_size != 0) && (waveBank->m_id == waveNo)) {
			return (reinterpret_cast<int>(waveBank) - m_waveBankBase) / REDSOUND_HISTORY_BANK_ENTRY_SIZE;
		}
		waveBank += 1;
	}
	...
	if (m_waveLoadNo < 0) {
		...
		if (historyNo >= 0) {
			if ((waveBankNo >= 0) && (historyNo != waveBankNo)) {
				*(int*)(m_waveBankBase + waveBankNo * REDSOUND_HISTORY_BANK_ENTRY_SIZE + 0x0) = *(int*)(m_waveBankBase + historyNo * REDSOUND_HISTORY_BANK_ENTRY_SIZE + 0x0);
				*(int*)(m_waveBankBase + waveBankNo * REDSOUND_HISTORY_BANK_ENTRY_SIZE + 0x4) = *(int*)(m_waveBankBase + historyNo * REDSOUND_HISTORY_BANK_ENTRY_SIZE + 0x4);
				*(int*)(m_waveBankBase + waveBankNo * REDSOUND_HISTORY_BANK_ENTRY_SIZE + 0x8) = *(int*)(m_waveBankBase + historyNo * REDSOUND_HISTORY_BANK_ENTRY_SIZE + 0x8);
				*(int*)(m_waveBankBase + waveBankNo * REDSOUND_HISTORY_BANK_ENTRY_SIZE + 0xC) = *(int*)(m_waveBankBase + historyNo * REDSOUND_HISTORY_BANK_ENTRY_SIZE + 0xC);
	...
	if (waveNo < 0) {
		if (waveNo == -1) {
			for (historyBank = reinterpret_cast<RedHistoryBANK*>(m_waveBankBase);
			     historyBank < reinterpret_cast<RedHistoryBANK*>(m_waveBankBase + REDSOUND_WAVE_BANK_SIZE); historyBank += 1) {
				if (historyBank->m_id >= 0) {
					WaveDelete(historyBank);
				}
			}
		} else if (waveNo == -2) {
			for (historyBank = reinterpret_cast<RedHistoryBANK*>(m_waveBankBase + REDSOUND_WAVE_HISTORY_BANK_OFFSET);
			     historyBank < reinterpret_cast<RedHistoryBANK*>(m_waveBankBase + REDSOUND_WAVE_BANK_SIZE); historyBank += 1) {
				if (historyBank->m_id >= 0) {
					WaveDelete(historyBank);
				}
		...
		if (waveNo >= 0) {
			WaveDelete(reinterpret_cast<RedHistoryBANK*>(m_waveBankBase + waveNo * REDSOUND_HISTORY_BANK_ENTRY_SIZE));
		}
	}
}
...
void CRedEntry::ClearWaveBank(int waveBankNo)
{
	...
	if (waveBankNo < 0) {
		...
		WaveDelete(reinterpret_cast<RedHistoryBANK*>(m_waveBankBase + waveBankNo * REDSOUND_HISTORY_BANK_ENTRY_SIZE));
	}
}
...
RedSeSepHEAD* CRedEntry::SeSepHeadAdd(RedSeSepHEAD* seSepHead)
{
	...
	if (bank < reinterpret_cast<RedHistoryBANK*>(m_seSepBankBase + REDSOUND_SESEP_BANK_SIZE)) {
	} else {
		bank = SeSepOldDelete();
	}
