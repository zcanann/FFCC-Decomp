...
{
    float yCoord = -1.0f;
    float stepX = 2.0f / (float)(image->m_width - 1);
    float stepY = 2.0f / (float)(image->m_height - 1);
    ...
    for (u32 y = 0; y < image->m_height; y++) {
        ...
        for (u32 x = 0; x < image->m_width; x++) {
            ...
            ImageBufferSetPixel_IA8(
                image, x, y,
                __cvt_fp2unsigned((double)(xCoord * magnitude * 127.0f + 128.0f)),
                __cvt_fp2unsigned((double)(yCoord * magnitude * 127.0f + 128.0f)), 0, 0);
            xCoord += stepX;
        }
