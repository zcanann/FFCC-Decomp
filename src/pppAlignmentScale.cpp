...
struct pppAlignmentScale* pppFrameAlignmentScale(struct pppAlignmentScale* alignmentScale, struct pppAlignmentScaleData* data)
{
    ...
    if (gPppCalcDisabled == 0) {
        ...
        if (FLOAT_80331920 < scale) {
            scale = (scale - FLOAT_80331920) * data->m_unk0x8 + FLOAT_80331920;
        } else {
        ...
        scale = FLOAT_80331924;
        pppMngStPtr->m_matrix.value[0][3] = FLOAT_80331924;
        pppMngStPtr->m_matrix.value[1][3] = scale;
        pppMngStPtr->m_matrix.value[2][3] = scale;
