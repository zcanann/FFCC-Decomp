...
enum RedMidiDataMapOffset {
    REDSOUND_MIDI_MAP_DATA_SIGN_TABLE_OFFSET = 0x000,
    REDSOUND_MIDI_MAP_DATA_SWING_FUNCTION_TABLE_OFFSET = 0x800,
    REDSOUND_MIDI_MAP_DATA_CONTROL_FUNCTION_TABLE_OFFSET = 0x840,
    REDSOUND_MIDI_MAP_DATA_SIZE = 0xA40,
};
...
STATIC_ASSERT(REDSOUND_SIGN_DATA_TABLE_OFFSET == REDSOUND_MIDI_MAP_DATA_SIGN_TABLE_OFFSET);
...
STATIC_ASSERT(REDSOUND_SWING_FUNCTION_TABLE_OFFSET == REDSOUND_MIDI_MAP_DATA_SWING_FUNCTION_TABLE_OFFSET);
...
STATIC_ASSERT(REDSOUND_MIDI_CONTROL_FUNCTION_TABLE_OFFSET ==
              REDSOUND_MIDI_MAP_DATA_CONTROL_FUNCTION_TABLE_OFFSET);
...
STATIC_ASSERT(REDSOUND_MIDI_CTRL_DATA_SIZE == REDSOUND_MIDI_MAP_DATA_SIZE);
...
int DataAddCompute(int* currentValue, int targetValue, int* stepCount)
{
    ...
    if (targetValue - (*currentValue >> REDSOUND_FIXED_SHIFT) != 0) {
        stepAdd = ((targetValue << REDSOUND_FIXED_SHIFT) | REDSOUND_FIXED_HALF) - *currentValue;
        stepAdd /= *stepCount;
    } else {
        *stepCount = 0;
    }
            ...
            for (; scan < RedSoundControlGetTrackEnd(control); scan++) {
                ...
                while ((currentDelta < 1) && (scan->m_command != REDSOUND_TRACK_COMMAND_NONE)) {
                    ...
                    if (scan->m_command != REDSOUND_TRACK_COMMAND_NONE) {
                        delta = RedTrackCommandReadDeltaTime(scan);
                        currentDelta += delta;
...
{
    RedControlRamp* reverbDepth = RedTrackReverbDepthGetRamp(track);
...
static void __MidiCtrl_WaveWithBank(RedSoundCONTROL*, RedKeyOnDATA*, RedTrackDATA* track)
{
	...
	if (waveBank != 0) {
		...
		track->m_waveData = (RedWaveDATA*)((u8*)waveBankData + waveTable[waveNo]);
...
static void __MidiCtrl_VolumeChange(RedSoundCONTROL*, RedKeyOnDATA*, RedTrackDATA* track)
{
    ...
    delta = RedTrackCommandReadDeltaTime(track);
    if (delta == 0) {
        delta++;
    }
...
{
	track->m_portamentTime = RedTrackCommandReadDeltaTime(track);
}
/*
 * --INFO--
...
static void __MidiCtrl_VibrateRateChange(RedSoundCONTROL*, RedKeyOnDATA*, RedTrackDATA* track)
{
    ...
    trackDelta = RedTrackCommandReadDeltaTime(track);
    if (trackDelta == 0) {
        trackDelta += 1;
    }
