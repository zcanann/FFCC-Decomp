 ...
 * JP Size: TODO
 */
void CPartMng::pppDrawPrioPdtFpno(unsigned char drawMode, short pdtId, short fpno)
{
    struct PppMngStDrawRaw {
        void* m_pppResSet;                   // 0x00
        int m_partIndex;                     // 0x04
    ...
    for (int i = 0; i < 0x180; i++, mng++) {
        if (mng->m_nodeIndex == fpno && mng->m_kind == pdtId && mng->m_endRequested == 0
            && mng->m_baseTime != -0x1000 && mng->m_drawMode == drawMode && mng->m_baseTime < 0) {
            if (mng->m_ownerVisible == 0) {
                return;
            }
 ...
 * JP Size: TODO
 */
void CPartMng::pppDrawIdx(int idx)
{
    struct PppMngStDrawRaw {
        void* m_pppResSet;                   // 0x00
        int m_partIndex;                     // 0x04
    ...
    PppMngStDrawRaw* mng =
        reinterpret_cast<PppMngStDrawRaw*>(reinterpret_cast<unsigned char*>(this) + 0x1D4 + idx * 0x158);
    if (mng->m_endRequested != 0) {
        return;
    }
