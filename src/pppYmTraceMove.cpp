...
void pppConstructYmTraceMove(pppYmTraceMove* pppYmTraceMove, pppYmTraceMoveUnkC* param_2)
{
	...
	savedPosX = pppMngStPtr->m_savedPosition.x;
	savedPosY = pppMngStPtr->m_savedPosition.y;
	savedPosZ = pppMngStPtr->m_savedPosition.z;
	paramVecX = pppMngStPtr->m_paramVec0.x;
	paramVecY = pppMngStPtr->m_paramVec0.y;
	paramVecZ = pppMngStPtr->m_paramVec0.z;
	pppSubVector__FR3Vec3Vec3Vec((Vec*)&dest[1].y, (Vec*)&savedPosX, (Vec*)&paramVecX);
	dir = *(Vec*)&dest[1].y;
...
void pppFrameYmTraceMove(pppYmTraceMove* pppYmTraceMove, pppYmTraceMoveUnkB* param_2, pppYmTraceMoveUnkC* param_3)
{
	...
	local_11c = pppMngSt->m_position;
	pppCopyVector__FR3Vec3Vec((Vec*)&pppMngSt->m_userFloat0, &local_11c);
