...
static const char s_cUsbPcsName[] = "CUSBPcs";
...
void CUSBPcs::Init()
{ 
	m_smallStage = Memory.CreateStage(0x2000, const_cast<char*>(s_cUsbPcsName), 0);
...
void CUSBPcs::IsBigAlloc(int param_2)
{
    if ((param_2 != 0) && (m_bigStage == (CMemory::CStage*)nullptr)) {
        m_bigStage = Memory.CreateStage(0x100000, const_cast<char*>(s_cUsbPcsName), 0);
