...
- **named-pointer anti-folding lever (NEW, +0.11 on wm DrawMainMenuSub — @NNN is NOT always cosmetic):** referencing an inline float/double constant via `const float* p = &FLOAT_x; ... *p` (instead of the bare `FLOAT_x`) forces MWCC to emit a NAMED `FLOAT_/DOUBLE_@sda21` reloc instead of interning the value into an anonymous `@NNN` pool slot. This DID raise the score on DrawMainMenuSub — so the earlier "@NNN is cosmetic / objdiff keys on value not name" conclusion is WRONG in at least some cases (the function-local-pool vs shared-symbol distinction can be a real text-fuzzy loss). Try this on functions whose only residual is @NNN-vs-named float relocs (measure — it sometimes reschedules and regresses, so gate per-edit).
