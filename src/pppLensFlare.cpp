...
void pppFrameLensFlare(pppColum* obj, pppColumUnkB* unkB, _pppCtrlTable* ctrlTable)
{
	if (gPppCalcDisabled == 0) {
		...
		u8* lensData = (u8*)obj + 0x80;
		u8* shapeBytes = lensData + shapeOffset;
		u8* colorBytes = lensData + colorOffset;
		u8* alphaPtr = shapeBytes + 0x32;
		u8 sourceAlpha = colorBytes[0x08];
		...
		GXProject((double)pppMngStPtr->m_matrix.value[0][3], (double)pppMngStPtr->m_matrix.value[1][3],
				  ...
				  (float*)(shapeBytes + 0x10), (float*)(shapeBytes + 0x14), (float*)(shapeBytes + 0x18));
		...
		*(float*)(shapeBytes + 0x34) = PSVECDotProduct(&cameraToObject, &lookDir);

		float xProjected = *(float*)(shapeBytes + 0x10);
		float yProjected = *(float*)(shapeBytes + 0x14);
		u8 argA = (u8)unkB->m_arg3;
		u32 halfWidth = (u32)(argA >> 1);
		u32 z0 = __cvt_fp2unsigned((double)(kPppLensFlareDepthToZScale * *(float*)(shapeBytes + 0x18)));
		...
		*alphaPtr = (u8)(int)((double)(float)(u8)*alphaPtr * alphaScale);
		if (unkB->m_dataValIndex != 0xffff) {
			...
			pppCalcFrameShape(*shapeTable, *(short*)(shapeBytes + 0x2c), *(short*)(shapeBytes + 0x2e),
							  *(short*)(shapeBytes + 0x30),
...
void pppRenderLensFlare(pppColum* obj, pppColumUnkB* unkB, _pppCtrlTable* ctrlTable)
{
	...
	lensData = (u8*)obj + 0x80;
	shapeBytes = lensData + shapeOffset;
	colorBytes = lensData + colorOffset;
	arg3Bytes = (u8*)&unkB->m_arg3;

	if ((unkB->m_dataValIndex != 0xffff) &&
		(shape = *(long**)(*(int*)&pppEnvStPtr->m_particleColors[0] + unkB->m_dataValIndex * 4), shapeBytes[0x32] != 0)) {
		pppCVECTOR local_70;
		Vec local_6c;
		Vec local_60;
		Mtx local_54;
		...
		local_54[0][0] = local_54[2][2] * pppMngStPtr->m_scale.x * *(float*)(lensData + 0x40);
		local_54[1][1] = local_54[2][2] * pppMngStPtr->m_scale.y * *(float*)(lensData + 0x54);
		local_54[2][2] = local_54[2][2] * pppMngStPtr->m_scale.z * *(float*)(lensData + 0x68);
		...
		pppCopyVector__FR3Vec3Vec((Vec*)(shapeBytes + 0x20), &local_6c);
		...
		local_70.rgba[3] = shapeBytes[0x32];
		...
		pppDrawShp(shape, *(s16*)(shapeBytes + 0x2e), pppEnvStPtr->m_materialSetPtr, arg3Bytes[2]);
		pppSetBlendMode(3);
	}
}
