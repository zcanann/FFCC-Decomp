// Function: drawShapeSeqGrouad__Fiiiiff8_GXColor8_GXColor8_GXColor8_GXColor
// Entry: 80158af4
// Size: 848 bytes

/* WARNING: Removing unreachable block (ram,0x80158e28) */
/* WARNING: Removing unreachable block (ram,0x80158e20) */
/* WARNING: Removing unreachable block (ram,0x80158b0c) */
/* WARNING: Removing unreachable block (ram,0x80158b04) */

void drawShapeSeqGrouad__Fiiiiff8_GXColor8_GXColor8_GXColor8_GXColor
               (double param_1,double param_2,int param_3,int param_4,uint param_5,uint param_6,
               undefined4 *param_7,undefined4 *param_8,undefined4 *param_9,undefined4 *param_10)

{
  int iVar1;
  undefined4 local_118;
  undefined4 local_114;
  undefined4 local_110;
  undefined4 local_10c;
  undefined4 local_108;
  undefined4 local_104;
  undefined4 local_100;
  undefined4 local_fc;
  undefined4 local_f8;
  float local_f4;
  undefined4 local_f0;
  undefined4 local_ec;
  float local_e8;
  undefined4 local_e4;
  undefined4 local_e0;
  float local_dc;
  undefined4 local_d8;
  undefined4 local_d4;
  float local_d0;
  Mtx local_cc;
  Mtx MStack_9c;
  undefined4 local_58;
  uint uStack_54;
  undefined4 local_50;
  uint uStack_4c;
  
  PSMTXIdentity(&local_cc);
  uStack_54 = param_5 ^ 0x80000000;
  uStack_4c = param_6 ^ 0x80000000;
  local_58 = 0x43300000;
  local_cc.value[0][3] = (float)((double)CONCAT44(0x43300000,uStack_54) - DOUBLE_80332d20);
  local_50 = 0x43300000;
  local_cc.value[0][0] = (float)param_1;
  local_cc.value[1][3] = (float)((double)CONCAT44(0x43300000,uStack_4c) - DOUBLE_80332d20);
  local_cc.value[1][1] = (float)-param_2;
  local_cc.value[2][2] = FLOAT_80332d78;
  GXLoadPosMtxImm(&local_cc,0);
  GXSetCurrentMtx(0);
  C_MTXOrtho(FLOAT_80332d9c,FLOAT_80332dec,FLOAT_80332d9c,FLOAT_80332df0,FLOAT_80332d9c,
             FLOAT_80332d28,&MStack_9c);
  MStack_9c.value[2][3] = MStack_9c.value[2][3] + FLOAT_80332d9c;
  GXSetProjection(&MStack_9c,1);
  iVar1 = **(int **)(*(int *)pppEnvStPtr->m_particleColors[0].m_colorRGBA + param_3 * 4);
  local_100 = g__shopMenu;
  iVar1 = iVar1 + *(short *)(iVar1 + param_4 * 8 + 0x10);
  MaterialMan._72_4_ = 0xace0f;
  MaterialMan._68_4_ = 0xffffffff;
  MaterialMan._76_1_ = 0xff;
  MaterialMan._284_4_ = 0;
  MaterialMan._288_4_ = 0x1e;
  MaterialMan._292_4_ = 0;
  MaterialMan._517_1_ = 0xff;
  MaterialMan._518_1_ = 0xff;
  MaterialMan._88_4_ = 0;
  MaterialMan._92_4_ = 0;
  MaterialMan._520_1_ = 0;
  MaterialMan._296_4_ = 0;
  MaterialMan._300_4_ = 0x1e;
  MaterialMan._304_4_ = 0;
  MaterialMan._64_4_ = 0xace0f;
  local_104 = DAT_80332d04;
  GXSetChanAmbColor(4,&local_104);
  local_108 = local_100;
  GXSetChanMatColor(4,&local_108);
  _GXSetBlendMode__F12_GXBlendMode14_GXBlendFactor14_GXBlendFactor10_GXLogicOp(1,4,5,5);
  _GXSetAlphaCompare__F10_GXCompareUc10_GXAlphaOp10_GXCompareUc(7,0,0,7,0xff);
  GXSetZCompLoc(1);
  MaterialMan._296_4_ = MaterialMan._284_4_;
  MaterialMan._300_4_ = MaterialMan._288_4_;
  MaterialMan._304_4_ = MaterialMan._292_4_;
  MaterialMan._64_4_ = MaterialMan._72_4_;
  SetMaterialMenu__12CMaterialManFP12CMaterialSetii
            (&MaterialMan,pppEnvStPtr->m_materialSetPtr,(uint)*(byte *)(iVar1 + 10),0);
  GXClearVtxDesc();
  GXSetVtxDesc(9,1);
  GXSetVtxDesc(0xb,1);
  GXSetVtxDesc(0xd,1);
  iVar1 = *(int *)(iVar1 + 0xc);
  local_d8 = *(undefined4 *)(iVar1 + 3);
  local_d4 = *(undefined4 *)(iVar1 + 7);
  local_e4 = *(undefined4 *)(iVar1 + 0x2b);
  local_e0 = *(undefined4 *)(iVar1 + 0x2f);
  local_d0 = FLOAT_80332d9c;
  local_dc = FLOAT_80332d9c;
  GXSetNumChans(1);
  GXSetChanCtrl(0,1,0,1,0,2,2);
  GXSetChanCtrl(2,1,0,1,0,2,2);
  local_118 = *param_10;
  local_114 = *param_9;
  local_110 = *param_8;
  local_10c = *param_7;
  local_fc = local_e4;
  local_f8 = local_e0;
  local_f4 = local_dc;
  local_f0 = local_d8;
  local_ec = local_d4;
  local_e8 = local_d0;
  RenderTexQuadGrouad__8CGraphicF3Vec3Vec8_GXColor8_GXColor8_GXColor8_GXColor
            (&Graphic,&local_f0,&local_fc,&local_10c,&local_110,&local_114,&local_118);
  return;
}

