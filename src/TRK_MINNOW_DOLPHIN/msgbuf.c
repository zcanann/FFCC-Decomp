...
/* 8044D8C0-8044F270 07A5E0 19B0+00 4/4 0/0 0/0 .bss             gTRKMessageBuffers */
...
DSError TRKInitializeMessageBuffers(void) {
    int i;
    for (i = 0; i < 3; i++) {
        TRKInitializeMutex(&gTRKMessageBuffers[i]);
        TRKAcquireMutex(&gTRKMessageBuffers[i]);
        TRKSetBufferUsed(&gTRKMessageBuffers[i], FALSE);
        TRKReleaseMutex(&gTRKMessageBuffers[i]);
    }
