...
static void _MusicCrossPlaySequence(int* command)
{
    ...
    if ((RedExecCommandArgGet(command, REDSOUND_MUSIC_COMMAND_ID) == control[REDSOUND_CONTROL_MUSIC_PRIMARY].m_musicId) ||
        (RedExecCommandArgGet(command, REDSOUND_MUSIC_COMMAND_ID) == control[REDSOUND_CONTROL_MUSIC_SKIP].m_musicId)) {
        return;
    }

    if (RedExecCommandArgGet(command, REDSOUND_MUSIC_COMMAND_ID) == control[REDSOUND_CONTROL_MUSIC_SECONDARY].m_musicId) {
        control[REDSOUND_CONTROL_MUSIC_PRIMARY].m_masterVolumeAdd =
            -control[REDSOUND_CONTROL_MUSIC_PRIMARY].m_masterVolume / RedExecCommandArgGet(command, REDSOUND_MUSIC_COMMAND_FADE_TIME);
        control[REDSOUND_CONTROL_MUSIC_PRIMARY].m_masterVolumeDelta = RedExecCommandArgGet(command, REDSOUND_MUSIC_COMMAND_FADE_TIME);
        ...
        control->m_masterVolumeAdd =
            ...
            RedExecCommandArgGet(command, REDSOUND_MUSIC_COMMAND_FADE_TIME);
        control->m_masterVolumeDelta = RedExecCommandArgGet(command, REDSOUND_MUSIC_COMMAND_FADE_TIME);
...
void CRedDriver::Init()
{
    ...
    do {
        nextSlot = slot + 1;
        seBlockSlots[slot] = REDSOUND_SE_BLOCK_DATA_NONE;
        slot = nextSlot;
    } while (nextSlot < REDSOUND_SE_BLOCK_BANK_COUNT);
    ...
    defaultVolume = REDSOUND_MASTER_VOLUME_FULL_FIXED;
    emptyMusicId = REDSOUND_MUSIC_ID_NONE;
    p_SoundControl[REDSOUND_CONTROL_SE].m_volume = defaultVolume;
    p_SoundControl[REDSOUND_CONTROL_MUSIC_SKIP].m_volume = defaultVolume;
    p_SoundControl[REDSOUND_CONTROL_MUSIC_SECONDARY].m_volume = defaultVolume;
    p_SoundControl[REDSOUND_CONTROL_MUSIC_PRIMARY].m_volume = defaultVolume;
    p_SoundControl[REDSOUND_CONTROL_SE].m_masterVolume = defaultVolume;
    p_SoundControl[REDSOUND_CONTROL_MUSIC_SKIP].m_masterVolume = defaultVolume;
    p_SoundControl[REDSOUND_CONTROL_MUSIC_SECONDARY].m_masterVolume = defaultVolume;
    p_SoundControl[REDSOUND_CONTROL_MUSIC_PRIMARY].m_masterVolume = defaultVolume;
    p_SoundControl[REDSOUND_CONTROL_MUSIC_SKIP].m_musicId = emptyMusicId;
    p_SoundControl[REDSOUND_CONTROL_MUSIC_SECONDARY].m_musicId = emptyMusicId;
    p_SoundControl[REDSOUND_CONTROL_MUSIC_PRIMARY].m_musicId = emptyMusicId;
    ...
    slot = 0;
    do {
        nextSlot = slot % REDSOUND_SE_VOICE_BASE_INDEX;
        RedVoiceDataGet(slot)->m_voiceIndex = nextSlot;
        slot = slot + 1;
    } while (slot < REDSOUND_VOICE_COUNT);
    ...
    nextSlot = 0;
    do {
        seTracks[nextSlot].m_trackNo = (char)(nextSlot + REDSOUND_SE_VOICE_BASE_INDEX);
        nextSlot = nextSlot + 1;
    } while (nextSlot < REDSOUND_SE_TRACK_COUNT);
