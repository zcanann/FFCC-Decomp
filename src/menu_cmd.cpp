...
static const float FLOAT_80332AD4 = 40.0f;
static const float FLOAT_80332ad8 = 0.9f;
static const double DOUBLE_80332AE0 = 0.015625;
static const float FLOAT_80332ae8 = 4.0f;
static const float FLOAT_80332AEC = 208.0f;
static const float FLOAT_80332AF0 = 2.0f;
static const double DOUBLE_80332af8 = 19.8;
static const double DOUBLE_80332B00 = 2.0;
static const float FLOAT_80332b08 = 320.0f;
static const float FLOAT_80332B0C = 3.0f;
...
static const double DOUBLE_80332B30 = 0.0;
...
void CMenuPcs::CmdDraw()
{
				...
				if (idx == list->count) {
					...
					if (tex == 0x37) {
						...
						if (sel < 2) {
							...
							if (canUse == 0) {
								rowTex = 0x34;
								rowAlpha = static_cast<float>(DOUBLE_80332a60) * row->alpha;
							}
						} else {
							s32 itemIdx = sel - 2;
							...
							if (equippable) {
								if (itemIdx + 2 < itemCount) {
									const float markX = rowX - FLOAT_80332b14;
									const float markY = (rowH - FLOAT_80332b18) *
					...
					rowColor.g = 0xFF;
					rowColor.b = 0xFF;
					rowColor.a = static_cast<u8>(FLOAT_80332acc * rowAlpha);
	...
	if (cmdMode == 1) {
		...
		listFont->SetMargin(FLOAT_80332a70);
		listFont->SetShadow(0);
		listFont->SetScale(FLOAT_80332ad8);
		listFont->DrawInit();
		...
		for (s32 row = 0; row < 8; row++) {
			...
			listFont->SetColor(
			    CColor(0xFF, 0xFF, 0xFF, static_cast<u8>(FLOAT_80332acc * alpha)).color);
		...
		for (s32 row = 0; row < 8; row++) {
			...
			if (displayIdx >= 2) {
				...
				DrawSingleIcon(
				    ...
				    static_cast<s32>(static_cast<float>(iconRow->y + 6) - FLOAT_80332a70),
	...
	if (cmdMode == 2) {
		...
		panelColor.g = 0xFF;
		panelColor.b = 0xFF;
		panelColor.a = static_cast<u8>(FLOAT_80332acc * panel->alpha);
		...
		DrawRect(
		    ...
		    panel->u, panel->v, FLOAT_80332a70, panel->scale, 0.0f);
		...
		if ((cmdMode == 0) || (cmdMode == 3)) {
			...
			if (cursorOnUnite) {
				cursorY = static_cast<float>(cursorEntry->y) +
				          (static_cast<float>(cursorEntry->height - 0x20) * static_cast<float>(DOUBLE_80332a60));
			} else {
				cursorY = static_cast<float>(cursorEntry->y);
				if (cmdMode != 0) {
...
unsigned int CMenuPcs::CmdCtrlCur()
{
	...
	if (mode == 0) {
		...
		int maxPos;
		if (DOUBLE_80332a58 == static_cast<double>(cmdList->entries[cmdList->listEnd + 3].scale)) {
			maxPos = 2;
		} else {
			maxPos = 3;
		}
	...
	case 3:
		for (s32 i = 0; i < static_cast<s32>(list->count); i++) {
			...
			if (static_cast<f64>(entry->alpha) < DOUBLE_80332a58) {
				entry->alpha = static_cast<f32>((DOUBLE_80332a68 * static_cast<f64>(GetCmdStateView(this)->transitionTimer)) + DOUBLE_80332a60);
				if (static_cast<f64>(entry->alpha) > DOUBLE_80332a58) {
					entry->alpha = FLOAT_80332a70;
				}
			}
