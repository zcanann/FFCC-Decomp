...
void CalcPolygonHeight(PYmMelt*, VERTEX_DATA* param_2, _GXColor* param_3, float param_4)
{
    ...
    for (i = 0; i < pointCount; i++, vertex++) {
        vertex->m_color.m_bytes[0] = param_3->r;
        vertex->m_color.m_bytes[1] = param_3->g;
        vertex->m_color.m_bytes[2] = param_3->b;
        vertex->m_color.m_bytes[3] = param_3->a;
...
void pppFrameYmMelt(PYmMelt* ymMelt, YmMeltCtrl* ctrl, PYmMeltDataOffsets* offsets)
{
    ...
    u16 gridSize = ctrl->m_initWOrk.m_gridSize;
    ...
    if (work->m_vertexData == nullptr) {
        ...
        int angleSeed = rand();
        s16 phaseDiv = ctrl->m_arg3.m_phaseDiv;
    ...
    if (ctrl->m_dataValIndex != 0xFFFF) {
        long** shapeTable = *(long***)(*(int*)&pppEnvStPtr->m_particleColors[0] + ctrl->m_dataValIndex * 4);
        pppCalcFrameShape__FPlRsRsRss(*shapeTable, work->m_shapeCurrentFrame,
                                      work->m_shapeDrawFrame, work->m_shapeFrameTime,
                                      ctrl->m_initWOrk.m_shapeStep);
    }
}
...
void pppRenderYmMelt(PYmMelt* ymMelt, YmMeltCtrl* ctrl, PYmMeltDataOffsets* offsets)
{
    ...
    pppSetDrawEnv__FP10pppCVECTORP10pppFMATRIXfUcUcUcUcUcUcUc(
        ...
        ctrl->m_payload[0x18], ctrl->m_arg3.m_blendMode, 2, 1, 1, 0);
    pppSetBlendMode(ctrl->m_arg3.m_blendMode);
    ...
    grid = ctrl->m_initWOrk.m_gridSize;
