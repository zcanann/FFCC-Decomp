...
int CFlatRuntime2::onClassSystemFunc(CFlatRuntime::CObject* object, int, int command, int& outResult)
{
	...
	switch (command) {
		...
		case -0x34:
			if (localBase[0] != 0) {
				engineObject->ResetAnimPoint(static_cast<int>(localBase[1]));
			}
			engineObject->AddAnimPoint(
			    static_cast<int>(localBase[1]),
			    static_cast<short>(localBase[3]),
			    static_cast<short>(localBase[2]));
			PushValue(this, object, 0);
			outResult = 0;
			break;
		case -0x35: {
		...
		case -0x3C:
			...
			break;
		case -0x41:
			engineObject->m_bgDownDist = 0.5f / static_cast<float>(static_cast<int>(localBase[0]));
			PushValue(this, object, 0);
			outResult = 0;
			break;
		case -0x43: {
