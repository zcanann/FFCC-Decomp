...
void CTexture::SetExternalTlut(void* tlutData, int loadToGX)
{
    ...
    int numEntries0 = kTextureC4TlutEntries;
    if (m_format == GX_TF_C8) {
        numEntries0 = kTextureC8TlutEntries;
    }
    GXInitTlutObj(&m_tlutObj0, tlutData, GX_TL_IA8, numEntries0);

    int numEntries1 = kTextureC4TlutEntries;
    if (m_format == GX_TF_C8) {
        numEntries1 = kTextureC8TlutEntries;
    }
    int tlutOffset = kTextureC4TlutEntries;
    if (m_format == GX_TF_C8) {
        tlutOffset = kTextureC8TlutEntries;
    }
    GXInitTlutObj(&m_tlutObj1, static_cast<u8*>(tlutData) + tlutOffset * 2, GX_TL_IA8, numEntries1);
