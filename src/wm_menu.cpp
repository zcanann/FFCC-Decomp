	...
	*reinterpret_cast<short*>(iVar12 + 0x20) = (short)iVar14;
	...
	case 2:
		if (m_wmWorldState->m_flag09 == 0) {
			...
			int iLines = iVar12 - (iVar12 >> 0x1F);
			if (iVar14 != (iVar12 - (iVar12 >> 0x1F)) * 0x16) iLines = iLines + 1;
	...
	case 0xC: {
		...
		if (m_menuWindowInfo->state == 1
			...
			LAB_saveIdx:
					...
					if (*reinterpret_cast<char*>(unk838 + 0x8A) == 0 && *reinterpret_cast<int*>(unk838 + 0x50) > 0) {
						...
						if (*reinterpret_cast<char*>(unk838 + 0xD2) == 0 && *reinterpret_cast<int*>(unk838 + 0x98) > 0) {
							...
							if (*reinterpret_cast<char*>(unk838 + 0x11A) == 0 && *reinterpret_cast<int*>(unk838 + 0xE0) > 0) {
								...
								m_mcCtrl.m_saveIndex = 3;
							}
						} else {
							m_mcCtrl.m_saveIndex = 2;
						}
					} else {
						m_mcCtrl.m_saveIndex = 1;
					}
				} else {
					m_mcCtrl.m_saveIndex = 0;
				}
				...
				unsigned int uVar11 = m_mcCtrl.m_serialLo;
				uVar15 = DAT_8032ee2c ^ m_mcCtrl.m_serialHi;
...
void CMenuPcs::DrawMainMenu()
{
	...
	do {
		if ((1u & (1 << frameIndex)) != 0) {
			short* entry = reinterpret_cast<short*>(m_wm.m_frameInfo + frameOffset + 4);
			MenuPcs.DrawRect(0xFFFFFFFF, static_cast<float>(entry[0]), static_cast<float>(entry[1]), static_cast<float>(entry[2]),
...
void CMenuPcs::DrawMCardMenu()
{
	...
	if (state > 0 && state < 4) {
		...
		if (alpha > DOUBLE_803314f0) {
			...
			GXColor bgColor = {0xFF, 0xFF, 0xFF,
			                   (unsigned char)(int)((double)FLOAT_80331458 * alpha)};
			GXSetChanMatColor(GX_COLOR0A0, bgColor);
			MenuPcs.SetTexture((TEX)0x1F);
			MenuPcs.DrawRect(0xFFFFFFFF, FLOAT_803313dc,
	...
	if (m_wmWorldState->m_mainState == 2 && m_wmWorldState->m_subState >= 0x11) {
		...
		double rdcY, rdcX;
		reinterpret_cast<int*>(&rdcY)[0] = 0x43300000;
		reinterpret_cast<int*>(&rdcY)[1] = (int)cursorY ^ 0x80000000;
		reinterpret_cast<int*>(&rdcX)[0] = 0x43300000;
		reinterpret_cast<int*>(&rdcX)[1] = (int)cursorX ^ 0x80000000;
		MenuPcs.DrawRect(0xFFFFFFFF, (float)(rdcY - DOUBLE_80331408), (float)(rdcX - DOUBLE_80331408),
	...
	if (state == 2 && m_wmWorldState->m_delay == 0) {
		...
		case 2:
			...
			if (winState == 1) {
				DrawMcWinMess(0, 0);
				double rawY;
				reinterpret_cast<int*>(&rawY)[0] = 0x43300000;
				reinterpret_cast<int*>(&rawY)[1] = ((int)m_menuWindowInfo->y + m_menuWindowInfo->height - 0x3e) ^ 0x80000000;
				float rectY = (float)(rawY - DOUBLE_80331408);
				int slotX = GetSlotABXPos((int)m_wmWorldState->m_cardChannel);
				double rawX;
				reinterpret_cast<int*>(&rawX)[0] = 0x43300000;
				reinterpret_cast<int*>(&rawX)[1] = slotX ^ 0x80000000;
				float rectX = (float)(rawX - DOUBLE_80331408);
				MenuPcs.SetAttrFmt((FMT)0);
				GXColor curColor = {0xFF, 0xFF, 0xFF, 0xFF};
				GXSetChanMatColor(GX_COLOR0A0, curColor);
				MenuPcs.SetTexture((TEX)0);
				double rdX, rdY;
				reinterpret_cast<int*>(&rdX)[0] = 0x43300000;
				reinterpret_cast<int*>(&rdX)[1] = (int)rectX ^ 0x80000000;
				reinterpret_cast<int*>(&rdY)[0] = 0x43300000;
				reinterpret_cast<int*>(&rdY)[1] = (int)rectY ^ 0x80000000;
				MenuPcs.DrawRect(0xFFFFFFFF, (float)(rdX - DOUBLE_80331408),
				         (float)(rdY - DOUBLE_80331408),
				         FLOAT_80331410, FLOAT_80331410,
				         FLOAT_803313dc, FLOAT_803313dc,
				         FLOAT_803313e8, FLOAT_803313e8, FLOAT_803313dc);
			}
		...
		case 0x19:
			...
			if (winState == 1) {
				...
				reinterpret_cast<int*>(&rawY)[1] = ((int)m_menuWindowInfo->y + m_menuWindowInfo->height - 0x3e) ^ 0x80000000;
				float rectY = (float)(rawY - DOUBLE_80331408);
				int ynX = GetYesNoXPos((int)m_wmWorldState->m_cardChannel);
				double rawX;
				reinterpret_cast<int*>(&rawX)[0] = 0x43300000;
				reinterpret_cast<int*>(&rawX)[1] = ynX ^ 0x80000000;
				float rectX = (float)(rawX - DOUBLE_80331408);
				if (m_wmWorldState->m_subState == 0x19) {
					...
					MenuPcs.SetAttrFmt((FMT)0);
					GXColor ynColor = {0xFF, 0xFF, 0xFF, 0xFF};
		...
		case 0x0C:
			...
			if (((winState != 1 ||
				...
				if (cRes == -1) m_wmWorldState->m_subState = 10;
					...
					BindMcObj();
					unsigned char* cs = m_wmCharaState;
					unsigned int idx = 0;
		...
		if (threshold <= m_wmWorldState->m_frameCounter) {
...
void CMenuPcs::DrawCMakeMenu()
{
	...
	if (m_wm.m_frameInfo != 0) {
		...
		for (int i = 0; i < 2; i++) {
			if (((1 << i) & 3) != 0) {
				unsigned char* const entry = m_wm.m_frameInfo + byteOffset + 4;
	...
	if (contentState > 0 && contentState < 4) {
		...
		if (m_wmWorldState->m_menuMode == 3) {
			...
			if (mainMode == 3) {
				...
				const int goOutMode = *(reinterpret_cast<char*>(g_pGoOutMenu) + 0x18);
				if (goOutMode == 0x0F) {
...
void CMenuPcs::DrawLoadMenu()
{
	...
	if (state > 0 && state < 4) {
		if (state == 1) {
			alpha = static_cast<float>(DOUBLE_803314E8 * (static_cast<double>(m_wmWorldState->m_frameCounter) - DOUBLE_80331408));
		} else if (state == 2) {
			alpha = FLOAT_803313e8;
		} else {
			alpha = static_cast<float>(-(DOUBLE_803314E8 * (static_cast<double>(m_wmWorldState->m_frameCounter) - DOUBLE_80331408) - DOUBLE_80331420));
		}
	...
	if (*reinterpret_cast<char*>(reinterpret_cast<int>(g_pGoOutMenu) + 0x18) == 0x0E) {
		...
		CColor color(0xFF, 0xFF, 0xFF, static_cast<unsigned char>(uAlpha));
		const int x = static_cast<int>(MenuPcs.CalcCenteringPos2(text, FLOAT_80331594, FLOAT_803313e8));
	...
	if (state == 2 && m_wmWorldState->m_subState >= 0x11) {
		...
		float cursorY1 = static_cast<float>(static_cast<double>(FLOAT_803314d8) + static_cast<double>(FLOAT_80331410) +
		                                    DOUBLE_80331498 * (static_cast<double>(saveIdx) - DOUBLE_80331408));
		...
		GXSetChanMatColor(GX_COLOR0A0, cursorColor);
		MenuPcs.SetTexture((TEX)0);
		MenuPcs.DrawRect(0xFFFFFFFF, (float)(int)cursorY0, (float)(int)cursorY1,
		         FLOAT_80331410, FLOAT_80331410,
		         FLOAT_803313dc, FLOAT_803313dc,
		         FLOAT_803313e8, FLOAT_803313e8, FLOAT_803313dc);
	...
	if (state == 2 && m_wmWorldState->m_delay == 0) {
		...
		case 2:
			...
			if (winState == 1) {
				DrawMcWinMess(0, 0);
				float slotY = (float)((m_menuWindowInfo->height - 0x3E) + m_menuWindowInfo->y);
				...
				MenuPcs.SetTexture((TEX)0);
				MenuPcs.DrawRect(0xFFFFFFFF, (float)(int)slotX, (float)(int)slotY,
				         FLOAT_80331410, FLOAT_80331410,
				         FLOAT_803313dc, FLOAT_803313dc,
				         FLOAT_803313e8, FLOAT_803313e8, FLOAT_803313dc);
			}
		...
		case 0x19:
			...
			if (winState == 1) {
				...
				// Yes/No cursor
				float ynY = (float)((m_menuWindowInfo->height - 0x3E) + m_menuWindowInfo->y);
				...
				if (m_wmWorldState->m_subState != 0x19) {
					...
					GXSetChanMatColor(GX_COLOR0A0, ynColor);
					MenuPcs.SetTexture((TEX)0);
					MenuPcs.DrawRect(0xFFFFFFFF, (float)(int)ynX, (float)(int)ynY,
					         FLOAT_80331410, FLOAT_80331410,
					         FLOAT_803313dc, FLOAT_803313dc,
					         FLOAT_803313e8, FLOAT_803313e8, FLOAT_803313dc);
				}
...
LAB_calc:
	...
	*reinterpret_cast<short*>(wmFrame + 0x98) =
	...
	unsigned int uVar14 = ((unsigned int)(((int)(uVar13 ^ 9) >> 1) - (int)((uVar13 ^ 9) & uVar13)) >> 0x1F) + 1;
	if ((int)uVar13 > 99) {
		uVar14 = 3;
	}
	...
	*reinterpret_cast<float*>(wmFrame + 0xE4) = fVar1;
	...
	const double dV9 = DOUBLE_80331408;
		...
		*reinterpret_cast<float*>(reinterpret_cast<int>(m_wm.m_frameData) + 0xC0) = fV4;
		...
		if (iVar19 >= 0) {
			do {
				...
				*reinterpret_cast<float*>(wmFrame + off + 0xBC) = (float)(dV10 * (double)(float)col);
				*reinterpret_cast<float*>(wmFrame + off + 0xC0) = (float)(dV12 * (double)(float)row + dV11);
...
void CMenuPcs::CalcMcObj()
{
	...
	const short panelStateValue = static_cast<short>(FLOAT_80331480);
	...
	for (int i = 0; i < 4; i++, panelState = reinterpret_cast<unsigned int*>(reinterpret_cast<unsigned char*>(panelState) + 0x50)) {
		*reinterpret_cast<short*>(panelState + 2) = panelStateValue;

		const int y = static_cast<int>(
		    static_cast<float>(yBase + ySlope * static_cast<double>(animCounter) + yOffset) -
		    yScale);
		...
		*reinterpret_cast<unsigned short*>(reinterpret_cast<unsigned char*>(panelState) + 0xE) = 0xE0;
		...
		panelState[1]++;
		if (static_cast<double>(static_cast<float>(panelState[1])) >=
