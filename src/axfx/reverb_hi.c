...
static const s32 axfx_reverb_hi_lens[8] = {
    0x000006FD, 0x000007CF, 0x0000091D, 0x000001B1,
...
static int ReverbHICreate(AXFX_REVHI_WORK* rv, f32 coloration, f32 time, f32 mix, f32 damping, f32 preDelay, f32 crosstalk) {
    ...
    for (k = 0; k < 3; k++) {
        ...
        for (i = 0; i < 2; i++) {
            DLcreate(&rv->AP[i + (k * 3)], axfx_reverb_hi_lens[i + 3] + 2);
            DLsetdelay(&rv->AP[i + (k * 3)], axfx_reverb_hi_lens[i + 3]);
        }

        DLcreate(&rv->AP[2 + (k * 3)], axfx_reverb_hi_lens[k + 5] + 2);
        DLsetdelay(&rv->AP[2 + (k * 3)], axfx_reverb_hi_lens[k + 5]);
    ...
    rv->damping = (axfx_reverb_hi_f32_0p8 * rv->damping) + axfx_reverb_hi_f32_0p05;
    rv->damping = axfx_reverb_hi_f32_1 - rv->damping;
...
static int ReverbHIModify(AXFX_REVHI_WORK* rv, f32 coloration, f32 time, f32 mix, f32 damping, f32 preDelay, f32 crosstalk) {
    ...
    rv->damping = (0.8f * rv->damping) + 0.05f;
    rv->damping = 1.0f - rv->damping;
