                                 ...
                                 _pppCtrlTable* param_3)
{
    if (gPppCalcDisabled == 0) {
        ...
        if ((gPppInConstructor != 1) && ((flags[1] != 0 || flags[0] != 0))) {
            ...
            float distanceBase = kPppConstrainCameraDir2DistanceBase;
            float scale = ((CameraPcs._252_4_ - distanceBase) / distanceBase) + kPppConstrainCameraDir2ScaleBase;

            PSMTXIdentity(pppMngStPtr->m_matrix.value);

            pppMngSt->m_scale.x = kPppConstrainCameraDir2ScaleMul * scale;
            pppMngSt->m_scale.y = scale;
            pppMngSt->m_scale.z = kPppConstrainCameraDir2ScaleBase;
