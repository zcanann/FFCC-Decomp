...
**MWCC saved-register RANKING RULE decoded (bonus_menu probe campaign):** callee-saved GPRs assign descending from r31 in this priority: (1) "maybe-undef" stealer webs — a var assigned in only one branch and read later under the same condition steals r31 and rotates the ring (fix: merge it with its else-branch twin into ONE variable defined on both paths); (2) `this`; (3) address-constant webs (anchor = lbl_*); (4) function-scope locals in DECLARATION order (verified to 8/11 register positions); (5) block-scope webs in creation order. UNSOLVED: some targets rank a hot loop-bound local ABOVE this+anchor — regime-dependent (other pragma sets flip it in our builds but destroy body codegen). Tooling: /tmp/apcwrap_regmap.sh, /tmp/apcwrap_webs.sh, /tmp/apcwrap_stateF.cpp (a CalcSelectCloseAnim body with the lower 8 regs already correct — any regime that flips the top-3 wins ~3pp).
