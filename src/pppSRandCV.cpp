...
void pppSRandCV(void* param1, void* param2, void* param3)
{
...

    {
        ...
        target_colors[0] += (s8)((f32)baseValue * target[0] - (f32)baseValue);
    }
    {
        s8 baseValue = in->delta[1];
        target_colors[1] += (s8)((f32)baseValue * target[1] - (f32)baseValue);
    }
    {
        s8 baseValue = in->delta[2];
        target_colors[2] += (s8)((f32)baseValue * target[2] - (f32)baseValue);
    }
    {
        s8 baseValue = in->delta[3];
        target_colors[3] += (s8)((f32)baseValue * target[3] - (f32)baseValue);
    }
}
