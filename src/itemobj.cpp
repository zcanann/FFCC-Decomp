...
{
	CFlatRuntime2* flat = reinterpret_cast<CFlatRuntime2*>(CFlat);

	for (CGItemObj* itemObj = flat->FindGItemObjFirst(); itemObj != 0;
	     itemObj = flat->FindGItemObjNext(itemObj)) {
...
{
	CGItemObj* itemObj = reinterpret_cast<CFlatRuntime2*>(CFlat)->FindGItemObjFirst();
	int deleteFlag = 1;
	CFlatRuntime2* flat = reinterpret_cast<CFlatRuntime2*>(CFlat);

	for (; itemObj != 0; itemObj = flat->FindGItemObjNext(itemObj)) {
		if (itemObj->m_owner == 0 &&
		    static_cast<signed char>(
		        static_cast<int>((static_cast<unsigned int>(itemObj->m_stateFlags0) << 28) & 0xC0000000) >> 31) != 0) {
			itemObj->m_flags = static_cast<unsigned char>(__rlwimi(itemObj->m_flags, deleteFlag, 7, 24, 24));
		}
	}
}
...
void CGItemObj::onHitParticle(int effectIndex, int, int, int, Vec*, PPPIFPARAM* hitParam)
{
	...
	if (worldParamA == 0xD || worldParamA == 0xE) {
		if (((particleAttr == 0 || particleAttr == 4) && worldParamA == 0xD) ||
			...
			reinterpret_cast<CFlatRuntime2*>(CFlat)->EndParticleSlot(*(int*)(self + 0x55C), 0);
			reinterpret_cast<CFlatRuntime2*>(CFlat)->ResetParticleWork(particleNo | 0x100, *(int*)(self + 0x55C));
			reinterpret_cast<CFlatRuntime2*>(CFlat)->SetParticleWorkPos(*(Vec*)(self + 0x15C), FLOAT_80331b20);
			reinterpret_cast<CFlatRuntime2*>(CFlat)->SetParticleWorkCol(9, 0, FLOAT_80331b18);
			reinterpret_cast<CFlatRuntime2*>(CFlat)->SetParticleWorkParam(classControl, this);
			reinterpret_cast<CFlatRuntime2*>(CFlat)->PutParticleWork();
		...
		if ((static_cast<unsigned int>(particleAttr - 0x66) <= 1U) || (particleAttr == 0x65)) {
			...
			if (classId != 0) {
				classObj = reinterpret_cast<CGObject*>(reinterpret_cast<CFlatRuntime2*>(CFlat)->intToClass(classId));
			} else {
				classObj = 0;
			}
	...
	reinterpret_cast<CFlatRuntime2*>(CFlat)->IgnoreParticle(effectIndex, this);
}
...
void CGItemObj::onFrameAlways()
{
	...
	if (*(int*)(self + 0x500) == 0xA) {
		...
		if (canUseTrace && *(int*)(CFlat + 66604) == 0) {
			*(int*)(CFlat + 66604) = reinterpret_cast<CFlatRuntime2*>(CFlat)->GetFreeParticleSlot();
			putParticleTrace(0x141, *(int*)(CFlat + 66604), this, FLOAT_80331b18, 0);
		} else if (!canUseTrace && *(int*)(CFlat + 66604) != 0) {
			reinterpret_cast<CFlatRuntime2*>(CFlat)->EndParticleSlot(*(int*)(CFlat + 66604), 0);
			*(int*)(CFlat + 66604) = 0;
		}
	}
}
...
{
	int freeItemCount = reinterpret_cast<CFlatRuntime2*>(CFlat)->getNumFreeObject(5);
	...
	if (freeItemCount == 0) {
		...
		for (unsigned char* itemObj = reinterpret_cast<unsigned char*>(reinterpret_cast<CFlatRuntime2*>(CFlat)->FindGItemObjFirst());
			 itemObj != 0;
		     itemObj = reinterpret_cast<unsigned char*>(
		         reinterpret_cast<CFlatRuntime2*>(CFlat)->FindGItemObjNext(reinterpret_cast<CGItemObj*>(itemObj)))) {
	...
	*reinterpret_cast<float*>(&inStack[4].m_word) = launchAngle;
	...
	CGPrgObj* newItem = 0;
	if (createMode != 1) {
		newItem = (CGPrgObj*)reinterpret_cast<CFlatRuntime2*>(CFlat)->intToClass((int)outStack.m_word);
...
{
	unsigned int numFreeObjects = reinterpret_cast<CFlatRuntime2*>(CFlat)->getNumFreeObject(5);
...
void CGItemObj::onFrameStat()
{
		...
		if (m_owner == 0 && (m_stateFlags0 & 8) != 0) {
			...
			if (*(int*)(self + 0x94) <= 0 || distance > DOUBLE_80331ba0) {
				...
				*(unsigned int*)(self + 0x1c0) = 1;
				reinterpret_cast<CFlatRuntime2*>(CFlat)->EndParticle(m_charaModelHandle);
				changeStat(9, 0, 0);
			}
		}
		break;
	...
	case 0xE:
		if (*(int*)(self + 0x528) == 0) {
			...
			reinterpret_cast<CFlatRuntime2*>(CFlat)->EndParticle(prgObj->m_charaModelHandle);
...
void CGItemObj::onDestroy()
{
	...
	reinterpret_cast<CFlatRuntime2*>(CFlat)->DeleteParticleSlot(*(int*)(self + 0x55c), 0);
	CGPrgObj::onDestroy();
}
...
void CGItemObj::onCreate()
{
	...
	m_particleSlot = reinterpret_cast<CFlatRuntime2*>(CFlat)->GetFreeParticleSlot();
}
