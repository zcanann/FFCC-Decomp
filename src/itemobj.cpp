...
{
	for (CGItemObj* itemObj = reinterpret_cast<CFlatRuntime2*>(CFlat)->FindGItemObjFirst(); itemObj != 0;
	     itemObj = reinterpret_cast<CFlatRuntime2*>(CFlat)->FindGItemObjNext(itemObj)) {
...
void CGItemObj::onHitParticle(int effectIndex, int, int, int, Vec*, PPPIFPARAM* hitParam)
{
	...
	int particleEntry = Game.unkCFlatData0[2] + hitParam->m_particleIndex * 0x48;
	int particleAttr = (int)*reinterpret_cast<unsigned short*>(particleEntry + 8);
	...
	if (worldParamA == 0xD || worldParamA == 0xE) {
		...
		if ((static_cast<unsigned int>(particleAttr - 0x66) <= 1U) || (particleAttr == 0x65)) {
			...
			unsigned int cid = static_cast<unsigned short>(classObj->GetCID());
